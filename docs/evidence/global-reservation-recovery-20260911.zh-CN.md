...
第二轮 Windows / Node 24.15.0 的完整测试为 473 通过、9 跳过、0 失败；随后进程清理烟测因子命令退出状态为 null 而失败。该测试的 PowerShell 命令上限为 15 秒，同轮 Windows / Node 24.x 的同一烟测成功耗时 13.84 秒。失败日志未保留 spawnSync.error，无法从中断言具体超时错误码；测试现直接检查该错误，并为包含 PowerShell 启动、Add-Type 编译和 CIM 查询的命令设置 60 秒上限。停止数量、目标进程实际退出和非目标进程仍存活的断言全部保留。
