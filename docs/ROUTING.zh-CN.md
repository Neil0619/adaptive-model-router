...
下表对应评分配置 v2。已有 v1 配置会继续把两个新增工作流权重视为 `0`，只有经过
显式确认的 re-anchor 才会启用 v2；历史 v1 配置和计分快照不会被改写。

| 信号 | 调整 |
| --- | ---: |
| 需求含糊、架构或权衡 | `+18` |
| 实际启用 `grill-with-docs` Skill | `+18` |
| 实际处于 Plan 模式 | `+18` |
...
`grill-with-docs` 与 Plan 模式是两个独立的事实信号，同时启用时叠加为
`+36`。仅在文本中提到它们不会加分；它们也不计入 Sol Max 所需的独立硬信号。
