...
import { verifiedRuntimeQualification } from "./lifecycle-qualification.mjs";
...
export function publishRuntime(db, candidate, dataRoot, { bootstrap = false, shellRoot = null, compatibilityProof = null, coldProof = null, legacyRuntime = null } = {}) {
  ...
  if (defaults && candidate.digest !== defaults.current_digest
    && !validRuntimeCompatibility(compatibilityProof, defaults.current_digest, candidate.digest)) {
    throw new Error("Exact A/B executable shared-writer qualification is required before publication");
  }
  for (const row of db.prepare("SELECT record FROM runtime_generations").all()) {
    const prior = JSON.parse(row.record);
    ...
    if (prior.writerDigest !== candidate.writerDigest || prior.shellDigest !== candidate.shellDigest
...
export function ensureRuntimeTask(db, context, { trustedHook = false, turnId = null, rootBirth = null } = {}) {
  ...
  if (!task) {
    ...
    // Dormant native tasks may have never called Router. Their first observed
    // call after installation is not their birth. Only a native root created
    // after the closed-host cutover can take the new default; missing metadata
    // conservatively preserves the legacy runtime. This never settles work.
    const generation = legacy && !(isNativeRootBirth(rootBirth) && rootBirth.time > legacy.cutoverTime)
      ? legacy.legacyDigest : defaults.current_digest;
...
export function bindRuntimeStage(db, context, routeId, invocation, qualification = false) {
  ...
  if (task.candidate && !qualification) throw new Error("Candidate qualification is unfinished; business admission remains paused");
...
export function beginRuntimeMigration(db, context, boundary) {
  const task = runtimeTask(db, context), defaults = publishedDefault(db);
  ...
  if (source.writerDigest !== candidate.writerDigest || source.shellDigest !== candidate.shellDigest) throw new Error("Task cannot cross this compatibility boundary");
